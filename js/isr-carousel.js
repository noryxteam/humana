(function () {
  const track = document.getElementById('isr-logo-track');
  if (!track) return;

  const viewport = track.parentElement;
  const prevBtn = document.querySelector('[data-isr-carousel-prev]');
  const nextBtn = document.querySelector('[data-isr-carousel-next]');
  const gap = 12;
  let index = 0;
